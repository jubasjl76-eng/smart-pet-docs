...
**Phase 18 — next:** **SHA-pin GitHub Actions repo-wide** (every workflow still uses `@vN` tags — do it uniformly with `ratchet`/`pin-github-action`); AWS posture (Security Hub / GuardDuty / scoped OIDC role) once the account is live; firmware static (cppcheck / clang-tidy / Flawfinder) + the Secure Boot / Flash Encryption design doc (rollout → Phase 19).
