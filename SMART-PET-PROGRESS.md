...
**Phase 18 — next (all blocked on the operator / AWS):** DAST (ZAP `-f openapi` + Nuclei) vs staging; AWS Security Hub / GuardDuty / IAM Access Analyzer baseline + scoped OIDC deploy role + ECR Enhanced Scanning; edge WAF rate-limiting; the weekly report's 2nd cycle (needs `SECURITY_REPORT_TOKEN`); external pen test (once customer data is live). Minor free follow-ups: OSV-Scanner for `platformio.ini` libs; SRI on the `/docs` Scalar CDN script; a repo-wide `SECURITY.md`; clang-tidy via a PlatformIO compile DB.
