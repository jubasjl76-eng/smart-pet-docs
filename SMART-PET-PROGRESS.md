...
**Phase 17 — remaining:** Wokwi command-loop test (needs `WOKWI_CLI_TOKEN` — `OPERATOR-ACTIONS.md` G4); Maestro-in-CI (emulator + EAS); native firmware coverage (gcov + PlatformIO link issue — deferred); fold the two web `e2e.yml`s into a `smart-pet-ci` reusable.
