...
| 14 — API contract & documentation | 🔨 in progress — backend route surface **fully on the zod OpenAPI registry** (141 paths); MQTT AsyncAPI done; api-client + shared registry done |
...
**Phase 14 — remaining (multi-session):** C++ structs/topics for the SDK from `asyncapi.yaml` (retires the hand-written `spd_topics.h` mirror); Starlight docs site; `smart-pet-backend` off `src/mqtt/contract.ts` → `@jubasjl76-eng/mqtt-contract` + tidy camera/sensors inline topic strings; the ~17 remaining camera routes onto the zod registry; dashboard adopts `@jubasjl76-eng/api-client` (retires hand-written `lib/api.ts`). `pet-iot-sensors-service` OpenAPI deferred — its proxy router (`src/routes/index.ts`) is dead code (never mounted); wire or remove it first.
| 13–21 | 📋 planned |
