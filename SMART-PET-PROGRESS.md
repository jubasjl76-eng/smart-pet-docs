...
| 17 — Test infrastructure | 🔨 in progress — **Playwright E2E** on the dashboard ([#17](https://github.com/jubasjl76-eng/backoffice-dashboard/pull/17) — auth guard + login + 3 authed pages, API mocked via `page.route`, `e2e.yml`) and the marketing site ([website#10](https://github.com/jubasjl76-eng/smart-pet-website/pull/10) — 9 tests, pt/en, page smoke, 404, inquiry form, runs on `mock/*.json`). |
| 18–21 | 📋 planned |

**Phase 17 — remaining:** firmware GoogleTest + coverage + sanitisers + Wokwi command-loop + contract test; backend API conformance test (running API vs `openapi.json`); Maestro + RNTL for `smart-pet-app`; reusable CI workflows.
