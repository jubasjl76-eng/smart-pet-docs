...
| 19 — Firmware hardening & HIL | 🔨 in progress — SDK: **time trust** (sdk#15), **brown-out resilience** (sdk#16), **fleet kill switch** (sdk#17 + backend#62 + dashboard#25), **memory guards** (sdk#18), **size gate** (ci#32 + sdk#19). |
| 20–21 | 📋 planned |
...
**Phase 19 — next (doable now):** OTA provenance (signed manifest with key-id + SHA in the `firmware` row, device-side hash verify — the S3 publish + SLSA-on-`.bin` need the AWS account); fleet observability screen in the dashboard. **Follow-up:** wire `pio-ci.yml` (+ `size-check`) into the 3 ported firmware repos (they only run static analysis in CI today); prod EMQX ACL must let a device *subscribe* to `kennel/{k}/_control` (outside its `kennel/{k}/{type}/{id}/#` subtree) — noted in `OPERATOR-ACTIONS.md`. **Blocked on hardware / operator:** Secure Boot + Flash Encryption rollout (design doc done, `security/secure-boot-flash-encryption.md` — pending sign-off + a pilot device); HIL bench.
