...
| 17 — Test infrastructure | 🔨 in progress — **Playwright E2E** (dashboard [#17](https://github.com/jubasjl76-eng/backoffice-dashboard/pull/17), website [website#10](https://github.com/jubasjl76-eng/smart-pet-website/pull/10), each with `e2e.yml`); **backend API conformance** ([#53](https://github.com/jubasjl76-eng/smart-pet-backend/pull/53) — `buildApp()` extracted; every OpenAPI op is proven mounted + `secure` ops reject unauth, 170 checks). |
| 18–21 | 📋 planned |

**Phase 17 — firmware (done):** `smart-pet-device-sdk` host tests migrated from the custom `CHECK` macro to **GoogleTest** — `test_topics` / `test_schedule` (fixtures) / `test_backoff` / `test_offline_journal` / `test_ulaw` + a new **`test_contract.cpp`** (golden topic scheme + QoS/retain policy from MQTT v2). New **`[env:native-san]`** runs the suite under ASan + UBSan. `smart-pet-ci/pio-ci.yml` `native` job now sets up PlatformIO + gcovr. ([sdk#9](https://github.com/jubasjl76-eng/smart-pet-device-sdk/pull/9), [ci#15](https://github.com/jubasjl76-eng/smart-pet-ci/pull/15))

**Phase 17 — remaining:** Wokwi command-loop test (needs `WOKWI_CLI_TOKEN` — `OPERATOR-ACTIONS.md` G4); native coverage (gcov + PlatformIO + lib_deps gtest don't link cleanly — deferred); Maestro + RNTL for `smart-pet-app`; fold the two web `e2e.yml`s into a `smart-pet-ci` reusable.
