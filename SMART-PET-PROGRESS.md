...
| 20 — Traffic control, caching & resilience | ✅ **DONE** — **Redis wired live** (backend#65, sensors#17, terraform#17), **rate-limit stack** (backend#66), **leader lock** (backend#67), **idempotency store** (backend#68), **SSE fan-out** (backend#69), **circuit breakers** (backend#70), **job queue + outbox** (backend#71), **table partitioning** (backend#72), **feature-flag mechanism** (backend#73, dashboard#27), **RDS Proxy + pool tuning** (backend#74, terraform#18), **MQTT broker limits** (terraform#19), **dashboard rate-limit UX + flag-gated rollout** (dashboard#28). |
| 21 — High availability, DR & load validation | 🔨 in progress — **EMQX cluster depth** (terraform#20, backend#75), **RDS read replica + read routing** (terraform#21, backend#76), **OTA CDN hardening** (sdk#21, backend#77, terraform#22), **ECS deploy safety** (terraform#23). |
...
**Phase 21 — ECS deploy safety (done):** the plan names 4 things; checked each against what `modules/ecs-service`/`modules/mqtt-broker` actually had. **≥2-AZ task spread** — already structural: `modules/network` always creates 2 AZs' worth of private subnets and Fargate balances a service's tasks across them automatically (Fargate doesn't even support the EC2-launch-type-only `ordered_placement_strategy`/`placement_constraints` to configure this explicitly). **`minimumHealthyPercent`/`maximumPercent`** — already 100/200 (zero-downtime rolling deploys). **Deployment circuit breaker (auto-rollback)** — genuinely missing; added (`enable = true, rollback = true`) to the backend/sensors service (`modules/ecs-service`) and the mqtt-broker's own service resource (matters more there now that terraform#20 made its task-def changes actually deploy). **Autoscale on request-count-per-target + MQTT-ingest-lag, not CPU alone** — only a CPU target-tracking policy existed; added a second `ALBRequestCountPerTarget` policy (coexists with CPU — ECS scales out on whichever wants more capacity), needing a new `alb_arn_suffix` input wired into all 3 envs × 2 services. `smart-pet-terraform` [#23](https://github.com/jubasjl76-eng/smart-pet-terraform/pull/23). **Scoped out, documented in the module header:** the custom MQTT-ingest-lag CloudWatch metric — nothing publishes prom-client metrics to CloudWatch today (Phase 16's Grafana Cloud dashboards-as-code is still the pending observability piece), so building a CloudWatch custom-metric pipeline just to feed one autoscaling policy would be infrastructure ahead of the foundation it should sit on.

**Phase 21 — OTA CDN hardening (done):** the plan names HTTP range-request/resumable OTA, immutable per-version paths, and treating a staged rollout as a CDN cache-warm curve. `smart-pet-device-sdk` [#21](https://github.com/jubasjl76-eng/smart-pet-device-sdk/pull/21): `OtaUpdater::applyFromUrl()` did a single-shot GET of the whole firmware image — any stall or drop discarded everything already written and restarted from byte 0. It now retries (up to 5 attempts) resuming with `Range: bytes=<written>-` from wherever the flash write got to, with the running SHA-256 accumulating across attempts so the final digest still covers the whole image; aborts cleanly if a resume attempt gets a plain 200 back (server ignored the Range header — the partial write can't be trusted to continue). New freestanding, host-testable helpers in `spd_ota_util.h` (`otaRangeHeader`, `otaRetryDelayMs`) — the rest of `OtaUpdater` needs the real ESP32 Arduino/HTTPClient/Update APIs, verified instead by building all 4 firmware envs (feeder/door/scale/audio) clean. `smart-pet-backend` [#77](https://github.com/jubasjl76-eng/smart-pet-backend/pull/77): `POST /api/breeder/fleet/firmware` now rejects a `url` that isn't `.../firmware/<deviceType>/<version>/...` — the free-text field had nothing stopping an admin from reusing a URL across versions, which would silently defeat CDN caching (a stale binary served from a warm edge after the S3 object underneath changed). `smart-pet-terraform` [#22](https://github.com/jubasjl76-eng/smart-pet-terraform/pull/22): `modules/cdn` gains `long_cache_paths` — `firmware/*` gets its own 1-year-TTL `ordered_cache_behavior` instead of the assets bucket's general 1-hour default, safe now that the path is enforced immutable; wired into all three envs (a longer TTL costs nothing extra, no per-env tradeoff needed). CloudFront already forwards `Range` + returns `206` for any S3-origin path with no extra config, so the SDK's resumable downloads needed nothing further here. **Scoped out, documented in `fleet.ts`:** a pull-based CDN-hosted `latest.json` manifest — devices already have a reliable push channel (MQTT + pg-boss retry/DLQ, Phase 20), and the backend has no S3-write capability at all today; building one just for a tiny manifest with no SDK-side poller either is a bigger lift than the fleet's current scale justifies.

**Phase 21 — RDS read replica + read routing (done):** the plan names a read replica specifically for exports / growth-chart aggregation / GDPR export, so those heavy, occasional reads don't compete with the primary's transactional load. `smart-pet-terraform` [#21](https://github.com/jubasjl76-eng/smart-pet-terraform/pull/21): `modules/database` gains `create_read_replica` (default off — a second always-on instance is real ongoing cost) provisioning an `aws_db_instance` with `replicate_source_db`, sharing the primary's VPC/security group/parameter group; enabled only in `envs/prod` (dev/staging stay off, same cost-discipline call as Phase 20's RDS Proxy deferral — those envs exercise the routing code via its primary-pool fallback instead of a live second replica). Also enabled `enabled_cloudwatch_logs_exports` on both the primary and the new replica (a Semgrep finding on the new resource, fixed on both rather than just the flagged one). `smart-pet-backend` [#76](https://github.com/jubasjl76-eng/smart-pet-backend/pull/76): `queryReplica()`/`queryOneReplica()` — a second `pg.Pool` against `PG_REPLICA_HOST`, transparently falling back to the primary pool when unset. Routed to it: GDPR export (`GET /api/breeder/privacy/export`), animal growth curves, and litter puppies-with-growth-series (one weight-history query per pup). Accepted trade-off, noted in the code: ordinary replication lag means a row written moments before one of these reads could be momentarily stale on the replica — none of the three are read-your-own-writes flows, so this wasn't worth a synchronous-replication requirement. New `readReplica.test.ts`.

**Phase 21 — EMQX cluster depth (done):** the plan names EMQX's clustering (shared session + retained state across nodes, split-brain policy) for A11 — the broker actually deployed (`modules/mqtt-broker`) is Mosquitto, which has no clustering protocol at all: N Mosquitto tasks behind the NLB are N fully independent replicas, not a cluster, so there's no shared distributed state to split-brain over in the first place. A real clustered/RocksDB retained store is an EMQX-only capability — same cost/complexity deferral as Phase 20's broker-limits decision (terraform#19), still not justified at this fleet's scale. What was a genuine, broker-agnostic gap: prod already runs `desired_count = 2`, but every task wrote its Mosquitto persistence file to the **same shared EFS mount** — two processes concurrently appending one file is a real corruption risk, not HA. `smart-pet-terraform` [#20](https://github.com/jubasjl76-eng/smart-pet-terraform/pull/20): each task now gets its own persistence subdirectory keyed by its ENI's private IP (unique per Fargate task); a loopback-only (`127.0.0.1:18883`, anonymous) listener backs a real ECS container health check (`mosquitto_pub` round-trip) the NLB's plain-TCP check can't provide, even in prod where the main listener requires auth; removed `lifecycle { ignore_changes = [task_definition] }` on the service — that block exists in `modules/ecs-service` because a separate CI pipeline registers new task defs out-of-band, but `modules/mqtt-broker` has no such pipeline (its image is a plain terraform variable), so it had been silently preventing every past config change to this module from ever reaching the running service. Losing a client's server-side session/retained state on failover to a different task is the accepted degradation the plan itself names for "broker down": devices journal locally and replay on reconnect, and `smart-pet-backend` [#75](https://github.com/jubasjl76-eng/smart-pet-backend/pull/75) now drops out-of-order replays — `applyStatus()` compares the device's own reported `timestamp` against a new `devices.last_status_ts` watermark and drops anything not strictly newer (with a carve-out for an LWT's wire-mandated `timestamp: 0`, which always applies but never moves the watermark backwards). New `applyStatus.test.ts` (in-order apply, stale replay dropped, duplicate-timestamp dropped, LWT-always-applies) against a real PGlite-backed `devices` table.

---

**Phase 20 — dashboard rate-limit UX + flag-gated rollout (done), closing out the phase:** `@jubasjl76-eng/api-client` has thrown a `RateLimitedError` (once its own retry/backoff on 429 is exhausted) since Phase 14, and the dashboard already turned it into a friendlier string — but the only place that string ever surfaced was a blocking `alert()` on mutations or a plain error string on queries, no retry affordance. Separately, Phase 20's feature-flag mechanism (backend#73, dashboard#27) had nothing in the codebase actually gated on a flag yet. `backoffice-dashboard` [#28](https://github.com/jubasjl76-eng/backoffice-dashboard/pull/28) closes both at once, using the new UX as the demonstration rollout: `src/lib/flags.ts` reads `GET /api/config`'s `flags` map once at load and polls it every 30s (an admin flipping a flag in Ops > Flags reaches every already-open tab, not just the next page load); `useQuery`/`useMutation` check `isFlagEnabled('dashboard-rate-limit-ux')` when they catch a `RateLimitedError` — off (default, no row in `feature_flags` yet) keeps today's `alert()`/plain-string behavior exactly as-is, on switches every tab to a new non-blocking `<RateLimitBanner>` (countdown from `retryAfterMs` + a "Retry now" button that re-runs the original request) via a small pub/sub (`rateLimitBus.ts`) since `useApi.ts` has no JSX of its own. The kill switch works in both directions — ship it dark, flip it on in the console when ready, flip it back off if it misbehaves.

**Phase 20 — MQTT broker limits (done):** the hardening plan names EMQX's per-client message-rate + connection-rate limiting for A10 abuse prevention, but the broker actually deployed (`modules/mqtt-broker`) is Mosquitto, not EMQX — migrating brokers is a real cost/complexity jump the fleet's current scale (zero real-world flooding incidents to date) doesn't justify, so that specific capability is deferred rather than faked. `smart-pet-terraform` [#19](https://github.com/jubasjl76-eng/smart-pet-terraform/pull/19) applies everything Mosquitto natively enforces instead: a per-listener `max_connections` ceiling, `max_queued_messages` (per-client outgoing queue cap before Mosquitto starts dropping), `message_size_limit` (device payloads are small JSON), and `max_keepalive` (Mosquitto's own default is effectively unlimited, letting a client stall dead-client detection indefinitely). A new `aws_cloudwatch_log_metric_filter` counts Mosquitto's own "dropped/exceeded/denied" log lines when a limit is breached, alarmed on a burst via each env's existing SNS alarm topic (staging + prod; skipped in dev, where breaches are expected/harmless during testing). The genuine gap — packets-per-second rate limiting — has no Mosquitto equivalent; noted in the module as the concrete trigger for revisiting an EMQX migration if a real incident ever justifies it.

**Phase 20 — RDS Proxy + pool tuning (done):** the hardening plan's own trigger for RDS Proxy is "matters at backend ×6" (prod's actual peak scale); at that scale even pg's own unmanaged `max: 10`-per-instance default stays comfortably under a small RDS instance's connection ceiling, so real RDS Proxy provisioning (an ongoing AWS cost) was deferred in favor of just sizing the pool intentionally instead of by accident. `smart-pet-backend` [#74](https://github.com/jubasjl76-eng/smart-pet-backend/pull/74): `PG_POOL_MAX`/`PG_IDLE_TIMEOUT_MS`/`PG_CONNECTION_TIMEOUT_MS`/`PG_STATEMENT_TIMEOUT_MS` wired into the main `pg.Pool`; also found and fixed that `pg-boss` builds its **own separate internal pool** and had been silently inheriting pg's own `max: 10` default the whole time — a previously invisible connection-budget consumer alongside the app's main pool — now sized explicitly via a new `PG_BOSS_POOL_MAX` (default 5, mostly idle between polls). `pg_pool_connections` gains a `max` state so a dashboard can graph utilization against the real ceiling, not just raw counts. `smart-pet-terraform` [#18](https://github.com/jubasjl76-eng/smart-pet-terraform/pull/18): `modules/database` gained explicit `max_connections` (parameter-group level, static — needs a reboot, `apply_method = "pending-reboot"`) and `statement_timeout_ms` (dynamic, applies immediately) variables instead of leaving both at RDS's own defaults; each env computes its backend's `PG_POOL_MAX` from the database's real `max_connections` divided by that env's actual `backend_max_count` (2 staging / 6 prod / 3 dev), reserving headroom for RDS's own overhead and pg-boss's pool, and passes it into the backend's `environment_vars` — so the pool ceiling is derived from real infra numbers, not a guessed constant. New `docs/OPERATOR-ACTIONS.md` H4: a `terraform apply` that changes `max_connections` needs a manual RDS reboot to take effect (static parameter).
