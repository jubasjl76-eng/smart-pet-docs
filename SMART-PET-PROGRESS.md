...
**Phase 16 — remaining:** `traceparent` in the MQTT envelope (cross-service tracing); Grafana Cloud dashboards-as-code in `smart-pet-terraform` (needs the account — `OPERATOR-ACTIONS.md` E1); `pino` structured logging; DB-pool tuning + circuit-breaker metrics (with Phase 20's `opossum`); runbooks + status page; cost budgets; Sentry replay/vitals review.
