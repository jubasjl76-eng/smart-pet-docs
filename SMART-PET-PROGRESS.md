...
| 19 — Firmware hardening & HIL | 🔨 in progress — **time trust** ([sdk#15](https://github.com/jubasjl76-eng/smart-pet-device-sdk/pull/15)): last-known-good epoch persisted to NVS + restored on boot, `timeTrusted()` gate on schedule execution, `clockOffsetS` telemetry. |
| 20–21 | 📋 planned |
...
**Phase 19 — next (doable now):** brown-out / power-loss resilience (NVS commit discipline, config CRC + factory-default fallback, `esp_brownout` handler — A12 #15); fleet kill switch (retained `kennel/{k}/_control` safe-mode topic + SDK honour + dashboard "halt fleet" — A12 #17); on-device memory guards (heap integrity in debug builds, stack budgets); size-regression CI gate + `puncover` PR report; OTA provenance (signed manifest with key-id + SHA in the `firmware` row, device-side hash verify — the S3 publish + SLSA-on-`.bin` need the AWS account); fleet observability screen in the dashboard. **Blocked on hardware / operator:** Secure Boot + Flash Encryption rollout (design doc done, `security/secure-boot-flash-encryption.md` — pending sign-off + a pilot device); HIL bench.
